            ...
            <form onSubmit={handleUpdate} className="space-y-8">
              ...
              <div className={`p-6 rounded-2xl transition-all duration-300 ${event?.plan_type === 'WHITE_LABEL' ? 'bg-bg-subtle border border-border' : 'bg-white/[0.02] border border-dashed border-black/5 dark:border-black/20 dark:border-black/10 dark:border-white/5 opacity-40 grayscale pointer-events-none'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-[10px] font-bold mb-3 uppercase tracking-widest text-text-secondary">Secondary Color</label>
                    <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-bg-subtle border border-border">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
                  <div>
                    <label className="block text-[10px] font-bold mb-3 uppercase tracking-widest text-text-secondary">Custom Domain</label>
                    <input type="text" value={customDomain} disabled={event?.plan_type !== 'WHITE_LABEL'} onChange={e => setCustomDomain(e.target.value)} placeholder="gallery.client.com" className="w-full px-4 py-3 rounded-xl bg-bg-subtle border border-border focus:outline-none transition-all text-sm" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold mb-3 uppercase tracking-widest text-text-secondary">Brand Logo URL</label>
                    <input type="text" value={logoUrl} disabled={event?.plan_type !== 'WHITE_LABEL'} onChange={e => setLogoUrl(e.target.value)} placeholder="https://domain.com/logo.png" className="w-full px-4 py-3 rounded-xl bg-bg-subtle border border-border focus:outline-none transition-all text-sm" />
                  </div>
                </div>
              </div>
              ...
              <div>
                <label className="flex items-center justify-between px-6 py-5 rounded-2xl bg-bg-subtle border border-border hover:bg-white/[0.08] transition-all cursor-pointer">
                  <div>
                    <span className="font-bold text-sm block mb-1">🛡️ Automatic Safety Filter</span>
                    <p className="text-[10px] text-text-secondary uppercase tracking-widest font-bold">Pro Edition Protection</p>
                  </div>
              ...
              <div className="p-6 rounded-2xl bg-bg-subtle border border-border">
                ...
                <input type="datetime-local" value={expiresAt || ''} onChange={e => setExpiresAt(e.target.value)} 
                  className="w-full px-5 py-3 rounded-xl bg-bg-subtle border border-border focus:outline-none focus:border-cyan-500/40 transition-all font-mono text-sm" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label className="flex items-center justify-between px-5 py-4 rounded-xl bg-bg-subtle border border-border hover:bg-white/[0.08] transition-all cursor-pointer">
                ...
                <label className="flex items-center justify-between px-5 py-4 rounded-xl bg-bg-subtle border border-border hover:bg-white/[0.08] transition-all cursor-pointer col-span-1 sm:col-span-2">
