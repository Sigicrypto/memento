...
async function compressDemoImage(file: File) {
  ...
  try {
    ...
    const blob = await new Promise<Blob | null>((res) => canvas.toBlob(res, outputType, 0.82));
    if (!blob || blob.size >= file.size) return file;
    const ext = outputType === 'image/png' ? 'png' : 'jpg';
    return new File([blob], file.name.replace(/\.[^.]+$/, `.${ext}`), { type: outputType, lastModified: Date.now() });
  } catch {
    return file;
  }
}

/** Broadcasts over a fresh channel guaranteed to be subscribed first */
async function broadcastUpload(demoId: string, payload: DemoMedia) {
  const ch = supabase.channel(`demo-wall-${demoId}`);
  return new Promise<void>((resolve) => {
    ...
    ch.subscribe(async (status) => {
      if (status === 'SUBSCRIBED') {
        try {
          await ch.send({ type: 'broadcast', event: 'NEW_UPLOAD', payload });
        } catch (_) {}
        // Give Supabase a moment to propagate before removing
        setTimeout(() => {
...
function UploadContent() {
  ...
  // Track cumulative count across multiple upload rounds (survives "Share Another")
  ...
  // Set page title
  useEffect(() => {
    document.title = 'Upload to Live Wall — Memento';
  }, []);

  // Persistent connection status channel (for UI indicator only)
  useEffect(() => {
    if (!demoId) return;
    const ch = supabase.channel(`demo-wall-status-${demoId}`);
  ...
  const handlePhotoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (spaceLeft <= 0) {
      setValidationMessage(`You can upload a maximum of ${MAX_IMAGES} images per round.`);
    } else {
      if (nextFiles.length > spaceLeft) setValidationMessage(`Only ${spaceLeft} more image(s) allowed this round.`);
  ...
  const handleVideoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (videos.length >= MAX_VIDEOS) {
      setValidationMessage(`Maximum ${MAX_VIDEOS} video per round.`);
      return;
    }
    setValidationMessage(null);
    ...
    try {
      for (const file of allFiles) {
        ...
        if (data) {
          ...
          // Save to local storage (works for same-browser cross-tab)
          upsertDemoPhoto(demoId, payload);

          // Broadcast via a guaranteed-subscribed channel (fixes stale closure bug)
          setStatus('Broadcasting to wall...');
  ...
  const handleShareAnother = () => {
    ...
    setPhotos([]);
    setVideos([]);
    // totalUploadedRef.current is preserved — totalUploaded state also stays
  };
      ...
      <div className="lp min-h-screen flex flex-col items-center justify-center py-12 px-4 relative">
        ...
        <div className="gcard w-full max-w-sm text-center">
          ...
          <div className="gcard-inner py-10">
            ...
            <p className="text-sm mb-2" style={{ color: 'var(--text2)' }}>Your photo is now live — check the big screen!</p>
            {totalUploaded > 0 && (
              <p className="text-xs font-bold mb-6" style={{ color: 'var(--amber)' }}>
                ✨ You've shared {totalUploaded} photo{totalUploaded !== 1 ? 's' : ''} this session
              </p>
            )}
            {lastUrl && (
  ...
  return (
    <div className="lp min-h-screen flex flex-col items-center py-20 px-4 relative">
      {/* Background */}
      <div className="orbs"><div className="orb orb1" /><div className="orb orb2" /><div className="orb orb3" /></div>
      ...
      <div
        ...
        style={{ background: 'rgba(255,255,255,0.75)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.8)', boxShadow: '0 4px 16px rgba(30,41,59,0.08)' }}
      >
        ...
        {totalUploaded > 0 && (
          <span className="text-xs font-bold ml-1" style={{ color: 'var(--amber)' }}>
            · {totalUploaded} shared ✓
          </span>
        )}
      </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                {photos.length < MAX_IMAGES && !uploading && (
                  <button
                    ...
                    style={{ background: 'rgba(245,158,11,0.06)', border: '2px dashed rgba(245,158,11,0.3)' }}
                  >
          ...
          <div className="gcard-inner">
            ...
            {videos.length === 0 ? (
              <button
                ...
                style={{ background: 'rgba(244,114,182,0.06)', border: '2px dashed rgba(244,114,182,0.3)' }}
              >
              ...
              <div className="relative aspect-video rounded-xl overflow-hidden" style={{ boxShadow: '0 4px 12px rgba(30,41,59,0.1)' }}>
                ...
                {!uploading && (
                  <button onClick={removeVideo} className="absolute top-2 right-2 w-8 h-8 flex items-center justify-center rounded-full font-bold text-white shadow-lg z-10" style={{ background: '#ef4444' }}>×</button>
                )}
              </div>
            )}
          </div>
      ...
      {(photos.length > 0 || videos.length > 0) && (
        <div
          ...
          style={{ background: 'rgba(255,255,255,0.88)', backdropFilter: 'blur(20px)', borderTop: '1px solid rgba(255,255,255,0.8)', boxShadow: '0 -4px 24px rgba(30,41,59,0.08)' }}
        >
          <div className="max-w-md mx-auto">
            ...
            <button
              ...
              style={{ padding: '1rem', fontSize: '1.1rem', opacity: uploading ? 0.7 : 1 }}
              disabled={uploading}
            >
