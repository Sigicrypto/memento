  ...
  (
    ...
    return (
      <button
        ...
        className={cn(
          "group relative z-0 inline-flex min-h-[38px] md:min-h-[40px] min-w-max cursor-pointer items-center justify-center overflow-hidden whitespace-nowrap border border-white/10 px-10 md:px-12 py-2 text-white [background:var(--bg)] [border-radius:var(--radius)] transform-gpu transition-all duration-300 ease-in-out hover:scale-[1.03] active:scale-[0.98] active:translate-y-px shadow-2xl disabled:pointer-events-none disabled:opacity-50 select-none",
          className
        )}
        ref={ref}
        {...props}
      >
        ...
        <span className="relative z-10 flex items-center justify-center gap-3 px-4 py-0.5 font-semibold text-xs md:text-sm tracking-wide leading-none">
          {children}
        </span>
