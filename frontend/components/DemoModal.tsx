  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 sm:p-8">
      ...
      <div className={`glass-panel w-full max-w-7xl h-full max-h-[900px] flex flex-col relative ${isFullscreen ? 'fixed inset-0 max-w-none max-h-none rounded-none' : ''}`} ref={modalRef}>
        ...
        <div className="h-20 flex items-center justify-between px-6 border-b border-border bg-surface/50">
           <div className="flex items-center gap-6">
              <AnimatedLogo width={140} height={40} />
              <div className="hidden sm:flex items-center gap-3">
                  <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-surface/50 border border-border">
                    <div className={`w-2 h-2 rounded-full ${isConnected ? 'bg-secondary' : 'bg-error'} ${isConnected ? 'animate-pulse' : ''}`} />
                    <span className="text-[10px] font-bold tracking-widest text-text-secondary">{isConnected ? 'LIVE CONNECTION' : 'RECONNECTING...'}</span>
                  </div>
                  <div className="px-3 py-1 rounded-full bg-surface/50 border border-border">
           ...
           <div className="flex items-center gap-4">
              ...
              }} className={`p-2 rounded-lg transition-all ${isAudioPlaying ? 'bg-primary text-text-primary shadow-lg' : 'text-text-muted hover:text-text-primary'}`}>
                {isAudioPlaying ? <Pause size={18} /> : <Music size={18} />}
              </button>
              <div className="flex bg-surface/50 p-1 rounded-xl">
                <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-primary text-text-primary shadow-lg' : 'text-text-muted hover:text-text-primary'}`}>
                  <Grid size={18} />
                </button>
                <button onClick={() => setViewMode('polaroid')} className={`p-2 rounded-lg transition-all ${viewMode === 'polaroid' ? 'bg-primary text-text-primary shadow-lg' : 'text-text-muted hover:text-text-primary'}`}>
                  <ImageIcon size={18} />
                </button>
                <button onClick={() => { setViewMode('slideshow'); setCurrentSlide(0); setIsPlaying(true); }} className={`p-2 rounded-lg transition-all ${viewMode === 'slideshow' ? 'bg-primary text-text-primary shadow-lg' : 'text-text-muted hover:text-text-primary'}`}>
                  <Play size={18} />
                </button>
              </div>
              ...
              <button onClick={onClose} className="p-2 text-text-muted hover:text-error transition-all">
                <X size={24} />
              </button>
           </div>
        </div>
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
            <div className="w-full md:w-80 border-b md:border-b-0 md:border-r border-border p-8 flex flex-col items-center bg-surface/50">
               ...
               <div className="p-4 bg-white rounded-2xl shadow-2xl">
                 {uploadUrl ? <QRCodeSVG value={uploadUrl} size={160} level="H" /> : <div className="w-40 h-40 bg-bg-subtle animate-pulse rounded-lg" />}
               </div>
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
                         {photos.map((photo, i) => (
                           <motion.div 
                             ...
                             className="group relative aspect-[4/5] rounded-3xl overflow-hidden border border-border hover:border-border-hover transition-all shadow-lg"
                           >
                              {photo.type === 'video' ? (
                              ...
                              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-4 flex flex-col justify-end">
                                 ...
                                 <div className="flex justify-between items-center mt-1">
                                   ...
                                   <button onClick={(e) => handleReaction(photo.id, e)} className="text-white/60 hover:text-accent-warm transition-colors flex items-center gap-1 text-[10px] font-bold">
                                     <Heart size={14} className={demoReactions[photo.id] ? 'fill-accent-warm text-accent-warm' : ''} /> {demoReactions[photo.id] || 0}
                                   </button>
                                 </div>
                              </div>
                           </motion.div>
                    ...
                    {viewMode === 'polaroid' && (
                      <div className="flex flex-wrap justify-center gap-10 py-10">
                         {photos.map((photo, i) => (
                           ...
                           >
                              <div className="aspect-[4/5] overflow-hidden bg-bg-subtle mb-4 rounded-sm relative">
                                {photo.type === 'video' ? (
                                  <video src={photo.url} className="w-full h-full object-cover" autoPlay muted loop playsInline preload="metadata" />
                                ) : (
                              ...
                              <p className="text-text-primary font-medium text-sm text-center font-handwriting line-clamp-2">{photo.caption}</p>
                              <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-4 text-[10px] font-bold text-text-muted opacity-0 group-hover:opacity-100 transition-opacity">
                                <span className="uppercase tracking-wider mt-0.5">BY {photo.uploader}</span>
                                <button onClick={(e) => handleReaction(photo.id, e)} className="hover:text-accent-warm transition-colors flex items-center gap-1">
                                  <Heart size={12} className={demoReactions[photo.id] ? 'fill-accent-warm text-accent-warm' : ''} /> {demoReactions[photo.id] || 0}
                                </button>
                              </div>
                           </motion.div>
                         ))}
                            ...
                            >
                              ...
                              <div className="absolute bottom-10 left-10 p-8 rounded-3xl bg-black/40 backdrop-blur-xl border border-border max-w-md">
                                 ...
                                 <div className="flex items-center gap-6">
                                   ...
                                   <button onClick={(e) => photos[currentSlide] && handleReaction(photos[currentSlide].id, e)} className="text-white/80 hover:text-accent-warm transition-colors flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full bg-white/10 border border-white/20">
                                     <Heart size={14} className={photos[currentSlide] && demoReactions[photos[currentSlide].id] ? 'fill-accent-warm text-accent-warm' : ''} /> {photos[currentSlide] ? (demoReactions[photos[currentSlide].id] || 0) : 0}
                                   </button>
                                 </div>
                              </div>
                           </motion.div>
