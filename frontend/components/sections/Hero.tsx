...
const Hero: React.FC<HeroProps> = ({ setIsDemoOpen }) => {
  return (
    <section className="relative overflow-hidden bg-bg min-h-screen pt-24 pb-12 lg:pt-0 lg:pb-0">
      <div className="lg:absolute lg:inset-0 grid grid-cols-1 lg:grid-cols-2 lg:h-full">
        {/* Left Panel: Hero Content */}
        <div className="relative z-10 flex flex-col justify-center px-6 sm:px-12 xl:px-20 py-12 lg:py-24">
          <motion.div
            ...
            className="max-w-2xl"
          >
            <div className="hero-badge mb-6 inline-flex">Live &middot; QR Photo Capture</div>

            <h1 className="text-5xl lg:text-6xl xl:text-7xl font-bold tracking-tight mb-6 leading-[1.1]">
              Collect Every Moment.
              <br />
              <span className="hero-title-accent text-gradient">Instantly.</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row items-start gap-4">
              ...
              <button className="relative group inline-flex items-center justify-center px-6 py-3 font-medium text-text-secondary hover:text-text-primary transition-colors">
                How it works 
        ...
        {/* Right Panel: Aurora + Beams + Mockup */}
        <div className="relative lg:flex flex-col justify-center bg-zinc-900 border-l border-white/5 overflow-hidden hidden">
          {/* Backgrounds */}
          ...
          <motion.div
            ...
            className="relative z-10 w-full max-w-xl mx-auto px-12"
          >
            <div className="viewfinder card p-2 rounded-2xl overflow-hidden shadow-2xl bg-bg-subtle border border-white/10 dark:border-white/10">
              <Corners />
              <div className="h-10 border-b border-border flex items-center px-4 gap-3 bg-surface">
                ...
                <div className="mx-auto w-40 sm:w-52 h-5 bg-bg-subtle border border-border rounded-md flex items-center justify-center">
              ...
              <div className="p-4 grid grid-cols-2 md:grid-cols-2 gap-4 bg-bg">
                {[
        ...
        {/* Mobile Mockup (Shown under text on small screens) */}
        <div className="relative flex lg:hidden flex-col justify-center mt-12 mb-12 px-6">
          <div className="absolute inset-0 -mx-6 bg-zinc-900 overflow-hidden rounded-3xl mx-4">
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1, delay: 0.2, ease: [0.16, 1, 0.3, 1] }}
            className="relative z-10 w-full max-w-sm mx-auto pt-10 pb-10"
          >
            <div className="viewfinder card p-2 rounded-2xl overflow-hidden shadow-2xl bg-bg-subtle border border-border">
              <Corners />
              <div className="h-8 border-b border-border flex items-center px-3 gap-2 bg-surface">
                <div className="flex items-center gap-1.5 text-accent-cyan">
                  <ScanLine size={12} strokeWidth={2} />
                  <span className="live-pulse" />
                </div>
              </div>
              <div className="p-3 grid grid-cols-2 gap-3 bg-bg">
                {[
                  { name: "Alex K.", color: "from-pink-500/20 via-rose-500/25 to-zinc-900/10", likes: 12 },
                  { name: "Jessica M.", color: "from-blue-600/20 via-indigo-500/25 to-zinc-900/10", likes: 8 },
                ].map((card, i) => (
                  <div key={i} className="viewfinder aspect-[4/5] bg-surface border border-border rounded-xl overflow-hidden relative group">
                     <Corners />
                     <div className={`w-full h-full bg-gradient-to-tr ${card.color}`} />
                  </div>
                ))}
              </div>
            </div>
