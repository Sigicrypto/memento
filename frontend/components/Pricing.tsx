  ...
  return (
    <section id="pricing" className={`${isEmbedded ? 'py-20' : 'pt-40 pb-32'} px-6 relative z-10 scroll-mt-32 w-full flex justify-center`}>
      <div className="container mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[28%_18%_18%_18%_18%] gap-6 lg:gap-0 mb-32 items-stretch w-full">
          {/* Empty column to perfectly align with the 28% Capability column in the table */}
          ...
          {PLANS.map((plan, idx) => {
            ...
            return (
              <div key={idx} className="w-full h-full lg:px-2 xl:px-3 flex">
                <motion.div 
                ...
                >
                  ...
                  <div className="flex items-center gap-3 mb-6">
                     ...
                     <h3 className="text-lg xl:text-xl font-bold">{plan.name}</h3>
                  </div>
   
                  <div className="mb-6">
                     <div className="flex items-baseline gap-1">
                        <span className="text-3xl xl:text-4xl font-bold tracking-tighter">{price}</span>
                        <span className="text-text-muted text-[10px] xl:text-xs font-medium">/event</span>
                     </div>
                     <p className="text-text-secondary text-[11px] mt-2 italic leading-snug">“{plan.description}”</p>
                  </div>
   
                  <div className="space-y-3 flex-grow mb-8">
                     <p className="text-[9px] xl:text-[10px] font-black uppercase tracking-widest text-primary mb-3">{plan.stats}</p>
                     {plan.features.map((f, i) => (
                        <div key={i} className={`flex items-start gap-2 text-[11px] ${f.included ? 'text-text-secondary' : 'text-text-muted opacity-40'}`}>
                           {f.included ? <Check size={12} className="text-primary mt-0.5 flex-shrink-0" /> : <X size={12} className="mt-0.5 flex-shrink-0" />}
                  ...
                  <a 
                     ...
                     className={`w-full py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all text-center mt-auto flex items-center justify-center gap-2 group ${plan.highlight ? 'bg-primary text-white shadow-xl shadow-primary/20' : 'bg-white/5 border border-white/10 hover:bg-white/10 text-white'}`}
                  >
